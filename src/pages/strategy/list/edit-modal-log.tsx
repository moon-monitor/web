...
import { DatasourceType, DictType, Status, StrategyType } from '@/api/enum'
...
import {
  ...
  parseLogStrategyDetailToFormData,
  updateStrategy
} from '@/api/strategy'
...
export interface LogEditModalProps extends ModalProps {
  strategyDetail?: StrategyItem
}

export const LogEditModal: React.FC<LogEditModalProps> = (props) => {
  ...
  useEffect(() => {
    if (restProps.open) {
      if (detail) {
        form.setFieldsValue(parseLogStrategyDetailToFormData(detail))
      } else {
        form.resetFields()
      }
    }
    ...
    <Modal {...restProps} onOk={handleSubmit} loading={detailLoading} confirmLoading={loading}>
      <div className='max-h-[70vh] overflow-y-auto overflow-x-hidden'>
        <Form form={form} layout='vertical' autoComplete='off' disabled={loading}>
          <Form.Item name='strategyType' initialValue={StrategyType.StrategyTypeLog} className='hidden'>
          ...
          <Form.Item label='查询语句' name='expr' rules={[{ required: true, message: '请输入服务地址' }]}>
          ...
          <Form.Item label={<b>告警等级</b>} required>
            <Form.List name='strategyLogLevels'>
              {(fields, { add, remove }) => (
                    ...
                    >
                      <Row gutter={12}>
                        ...
                        <Col span={6}>
                          ...
                          >
                            <InputNumber className='w-full' placeholder='请输入总数' />
                          </Form.Item>
                        </Col>
                      </Row>
                      <Row>
